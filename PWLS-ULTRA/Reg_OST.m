classdef Reg_OST < handle
    ...
    methods
        function obj = Reg_OST(mask, ImgSiz, PatSiz, SldDist, beta, gamma, mOmega, numBlock, CluInt)
            obj.mMask = mask;
            obj.PatSiz = PatSiz;
            obj.ImgSiz = ImgSiz;
            obj.SldDist = SldDist;
        ...
        function cost = penal(obj, A, x, wi, sino)
            ...
            clear mPat
            % sparsity error
            spa_err = obj.beta * sum( sum((mCod - obj.mSpa).^2,1) ); clear mCod;
            fprintf('se = %g\n', spa_err);
            spa = obj.beta * obj.gamma^2 * nnz(obj.mSpa); % L0 norm
            fprintf('sp = %g\n', spa);
