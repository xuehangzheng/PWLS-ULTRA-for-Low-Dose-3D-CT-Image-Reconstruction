function [D,Gamma,spar_err, err] = ksvd(params,varargin)
...
for iter = 1:iternum
  ...
  if (comperr)
   [spar_err(iter), err(iter)] = compute_err(D,Gamma,data);
  end
  if (testgen)
    if (memusage >= MEM_NORMAL)
      G = D'*D;
...
function [spar_err, err] = compute_err(D,Gamma,data)
...
if (codemode == CODE_SPARSITY)
  ...
  spar_err = sqrt(sum(col(data - D*Gamma).^2)/size(data,2));
  fprintf('||X_i - D*GAMMA_i||_2 normalized spar_err: = %g, ', spar_err);
  err = nnz(Gamma)/numel(Gamma);
  fprintf('sparsity = %g%%\n', err*100);  
end
